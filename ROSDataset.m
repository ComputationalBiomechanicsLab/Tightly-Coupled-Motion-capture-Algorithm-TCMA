classdef ROSDataset < AbstractObject
    % ROSDATASET - Class to hold all other dataset objects when available. Also
    ...
    properties (Description = 'User')
        Name
    end
    
    %% Public methods
    methods
        function obj = ROSDataset(varargin)
            % ROSDataset constructor
            obj = obj@AbstractObject(varargin{:});
        end
        
        syncMocap2Robot(obj,tracker,q_idx,T_sync)
        syncIMUs2Robot(obj,tracker,q_idx,T_sync)
        [delay,fig] = syncForce2Robot(obj,f_idx,q_idx,T_sync)
        
        newObj = interpolateAndResample(obj,fs)
        %newObj = cut(obj,T)
