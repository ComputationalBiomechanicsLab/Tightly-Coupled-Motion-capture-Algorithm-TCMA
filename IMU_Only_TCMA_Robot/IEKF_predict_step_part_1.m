function IEKF = IEKF_predict_step_part_1(IEKF, osimModel, state, ...
    torque1Gen, torque2Gen, torque3Gen, torque4Gen, torque5Gen, torque6Gen, t)
% IEKF_PREDICT_STEP_PART_1 Set prior states and compute Jacobians F_prev and L_prev.
%   Computes the continuous-time Jacobian G = dg/dx via numerical perturbation of the
%   OpenSim equations of motion, then discretizes via matrix exponential: F = expm(G * dt).
%
%   Inputs:
...
%
%   Output:
%       IEKF       - Updated struct with F_prev, L_prev, q_prev, u_prev, tau_prev
    ...
    % L_prev: discretized identity (additive process noise assumption)
